...
public class BFGovernanceService {
    ...
    private final BFGovernanceDescriptionTransformer governanceDescriptionTransformer;
    ...
    public BFProposalDto getProposal(String txHash, int index) {
        return storageReader.findProposalByTxHashAndIndex(txHash, index)
                .map(row -> {
                    ...
                    dto.setGovernanceDescription(
                            governanceDescriptionTransformer.transform(row.getType(), row.getDetails()));
                    return dto;
                })
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
