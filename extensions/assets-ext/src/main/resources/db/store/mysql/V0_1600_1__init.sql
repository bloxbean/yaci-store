...
CREATE TABLE cip26_metadata (
    ...
    decimals       INTEGER,
    -- base64-encoded image, up to ~87400 chars per CIP-26 — kept as LONGTEXT
    -- (MySQL TEXT is only 64 KB). Merged in here (was a separate ft_offchain_logo).
    logo           LONGTEXT,
    updated        TIMESTAMP NULL,
    updated_by     VARCHAR(255),
    -- Catch-all JSON. Holds the full original Mapping JSON with per-property
...
CREATE TABLE cip68_metadata (
    ...
    -- Catch-all JSON. Holds the parsed datum's non-scalar bits:
