...
- `prepare-range`
  ...
  - hiện tại chưa có `--force-rebuild`, nên rerun `prepare-range` không tự build lại cache cũ
- `compare-range`
...
- nếu bạn muốn build lại cache của `prepare-range` cho một epoch đã có cache, hiện tại cần xóa cache debug của epoch đó trước khi chạy lại
