...
### 5. Nếu chạy lặp lại `prepare-range` hoặc `compare-range` thì có bị lỗi không?

Thông thường là không.

Tool được thiết kế để rerun an toàn cho cùng một epoch hoặc một dải epoch:

- `prepare-range`
  - nếu cache cho epoch đã tồn tại thì tool sẽ coi đó là cache hit và reuse lại
  - nếu cần rebuild cache theo logic mới, hãy chạy `invalidate-cache` trước
- `compare-range`
  - luôn rebuild shadow snapshot trong temp tables của session hiện tại
  - luôn `DELETE` rồi `INSERT` lại dữ liệu của `snapshot_epoch` trong `drep_debug.shadow_drep_dist`

Điều này có nghĩa là:

- chạy lặp lại cùng epoch không tạo duplicate
- chạy lại một dải epoch con, ví dụ `624-630`, cũng không làm hỏng dữ liệu của epoch khác ngoài dải đó
- có thể rerun `compare-range` sau khi chỉnh SQL/debug logic để lấy bản shadow mới nhất
- có thể dùng `invalidate-cache` để xóa cache debug của một epoch/range trước khi chạy lại `prepare-range`

Điểm cần lưu ý duy nhất là hiệu năng:

- nếu bạn rerun `prepare-range` trên cùng range quá nhiều lần thì sẽ chỉ tốn thời gian DB, nhưng không làm sai dữ liệu
- nếu chạy song song nhiều compare cho cùng một `snapshot_epoch`, bản ghi cuối cùng trong `drep_debug.shadow_drep_dist` sẽ là bản của run hoàn tất sau cùng
...
### 5. Invalidate cache khi cần rebuild

Nếu bạn vừa sửa logic ảnh hưởng tới cache của `prepare-range`, có thể xóa cache
theo epoch/range rồi chạy lại:

```bash
yaci-store/bin/drep-dist-batch-compare.sh invalidate-cache \
  --epochs 508-624 \
  --cache-name pv9 \
  --workers 1
```

Các lựa chọn cho `--cache-name`:
