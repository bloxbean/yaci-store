...
def export_epoch_partitioned(pg_conn, table_name, config, output_dir):
...
def export_simple(pg_conn, table_name, config, output_dir):
...
def export_table(pg_conn, table_name, config, output_dir):
    """Export a single table and print summary."""
    log(f"Exporting {table_name}...")
    start = time.time()

    if table_name in EPOCH_PARTITIONED:
        row_count = export_epoch_partitioned(pg_conn, table_name, config, output_dir)
    else:
        row_count = export_simple(pg_conn, table_name, config, output_dir)
...
Examples:
  ...
  python3 export_dbsync_parquet_simple.py --env-file .env --tables drep_hash
  python3 export_dbsync_parquet_simple.py --pg-host localhost --pg-user admin \\
    ...
    try:
        for table_name in args.tables:
            export_table(pg_conn, table_name, TABLE_CONFIGS[table_name], output_dir)
    finally:
        pg_conn.close()
