...
```bash
# Start sync without indexes, wait for completion, then:
apply-indexes
```

This approach speeds up initial sync significantly, especially for mainnet.

### 2. Verify Indexes Regularly

```bash
# Include in maintenance routines
verify-indexes
```

### 3. Balance Index Usage
