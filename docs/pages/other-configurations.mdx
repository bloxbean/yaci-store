...
### Transaction Witness Persistence

The transaction store supports configurable witness persistence. This feature allows you to enable or disable the persistence of transaction witnesses in the database, which can help reduce storage requirements if witness data is not needed for your use case.
By default, transaction witness persistence is enabled.

```properties
store.transaction.witness-persistence-enabled=true
```
## Auto Sync off
