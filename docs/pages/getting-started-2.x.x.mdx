...
> 🔎 Mismatch details for all public networks can be found [here](./preview-drep-distr-mismatch-summary.mdx).
>
...
- Edit `yaci-store.yml` or `yaci-store-ledger-state.yml` to comment the database configuration in docker compose yml.monolith
