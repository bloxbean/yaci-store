...
Government action proposal statuses are correctly calculated up to recent epochs and are matching. However, there are some known mismatches in dRep distribution and dRep `active_until` values. We will be working on fixing these in future releases.
