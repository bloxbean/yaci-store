...
- **Preprod**:
  - ADAPots and rewards match up to epoch 226 (current epoch at the time of writing).

- **Preview**:
  - ADAPots and rewards match up to epoch 986 (current epoch at the time of writing).

A few dRep distribution mismatches (for 6 dReps) were found on the Preview network during some earlier epochs.

> 🔎 Mismatch details for all public networks can be found [here](./ledger-state-mismatches/2-0-0-beta1.mdx).
>
> ⚠️ We also identified **some incorrect `DRep active_until` values** (active_until: The epoch until which this drep is active) in Preprod and Preview. See [here](./ledger-state-mismatches.mdx)..
>
