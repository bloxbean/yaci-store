...
When the analytics store is enabled, DuckDB memory usage should be explicitly configured to prevent OOM issues in production and container environments. See the [Analytics Store Configuration Reference](/docs/v3/analytics/configuration#duckdb-memory-usage-in-jvm) for details.
