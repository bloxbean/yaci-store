...
Also, configure the [DuckDB memory limit](/docs/v3/analytics/configuration#duckdb-memory-usage-in-jvm) to prevent OOM issues, especially in containerized environments.
...
- **FAILED**: export failed; gap detection automatically retries it on the next scheduler run. Use the [admin API](/docs/v3/analytics/configuration#admin-api) to manually reset if needed.
