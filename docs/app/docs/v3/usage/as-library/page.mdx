...
- Java 21
- Spring Boot 3.5.x (Also tested with Spring Boot 3.3.x)
...
## Optional Indexes During Initial Sync

By default, Yaci Store does not create optional indexes during the initial sync to speed up the process.
However, if your application requires these indexes for better query performance or you are using an out-of-the-box API which needs an optional index, you can
create these indexes after the initial sync is complete programmatically, manually, or using Yaci Store Admin CLI.
