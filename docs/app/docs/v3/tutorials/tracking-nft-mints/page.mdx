...
<Callout type="info">
New to the plugin framework? You may want to skim the [Write Your First Plugin](/docs/v3/plugins/write-first-plugin) guide first. If you've already done the [Tracking UTXOs for a Specific Address](/docs/v3/tutorials/tracking-address-utxos) tutorial, this one is even simpler — it uses inline MVEL expressions instead of JavaScript files.
</Callout>
...
- **`asset.save`** — runs for every native-asset mint/burn (domain object [`TxAsset`](/docs/v3/plugins/plugin-api-guide#txasset): `policy`, `assetName`, `quantity`, `mintType`)
- **`metadata.save`** — runs for every transaction metadata label (domain object [`TxMetadataLabel`](/docs/v3/plugins/plugin-api-guide#txmetadatalabel): `label`, `body`)
...
### Learn more
- **[Plugin API Reference](/docs/v3/plugins/plugin-api-guide)** — every extension point and domain field, including [`asset.save`](/docs/v3/plugins/plugin-api-guide#assetsave) and [`metadata.save`](/docs/v3/plugins/plugin-api-guide#metadatasave)
- **[Write Your First Plugin](/docs/v3/plugins/write-first-plugin)** — filters, pre/post-actions, event handlers, and schedulers
- **[Governance Watch tutorial](/docs/v3/tutorials/governance-watch)** — scope-index Conway-era proposals and votes

### Try these modifications
- Send a Discord/webhook notification on each new mint using an event handler and the [HTTP client](/docs/v3/plugins/variables-reference-guide/http-client) (see the notification step in the [UTXO tutorial](/docs/v3/tutorials/tracking-address-utxos))
