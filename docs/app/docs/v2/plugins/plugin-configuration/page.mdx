...
Java plugins (available in 3.x only) can be used for any plugin type by setting `lang: java` and `class-name` to a fully qualified class name. 
The class should implement the matching plugin interface or extend one of the Java base classes (`JavaFilterPlugin`, 
`JavaPreActionPlugin`, `JavaPostActionPlugin`, `JavaEventHandlerPlugin`, `JavaInitPlugin`, or `JavaSchedulerPlugin`) 
and expose a `(PluginDef, PluginType, PluginContext)` constructor.
...
- **`lang`**: Plugin language (`mvel`, `spel`, `java`, `js`, `python`)
...
- **`class-name`**: Fully qualified Java class name for loading Java plugins
...
| `lang` | String | Yes | Plugin language: `js` (JavaScript), `mvel`, `python`, or `java` |
| `inline-script` | String | Yes* | Inline script code to execute (script languages only) |
| `script` | String | Yes* | Path to external script file (alternative to `inline-script`, script languages only) |
| `class-name` | String | Yes** | Fully qualified Java class name (`lang: java` only) |
...
- For script-based schedulers, either `inline-script` or `script` must be provided.
- For Java schedulers, `class-name` must be provided.
...
### Java Configuration
An example for plugins using yaci-store as a library can be found
[here](https://github.com/bloxbean/yaci-store/blob/main/examples/plugins/java/custom-indexer-with-plugins/src/main/java/com/bloxbean/example/ExpressionFilterPlugin.java).
```yaml
store:
  plugins:
    filters:
      metadata.save:
        - name: "Java Expression Filter"
          lang: java
          class-name: com.bloxbean.example.ExpressionFilterPlugin

        - name: "Java Collection Filter"
          lang: java
...
#### Java Features
- Works for filters, pre-actions, post-actions, event handlers, initializers, and schedulers
- Ideal for projects integrating yaci-store as a library
- Typed access to plugin context using class `PluginContext`
- Can be bundled in a custom Spring Boot app or loaded from an external JAR on the application classpath
