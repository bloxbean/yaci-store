...
This reference guide provides comprehensive documentation for all extension points, domain classes, and variables
available to plugin developers in the yaci-store system. The primary focus is on MVEL plugins with additional examples
in JavaScript, Python, and Java.
...
- **Parameter Signatures**:
  ...
  - Variables are directly available by name in script plugins (no context parameter)
  - Java plugins receive a typed `PluginContext` in their constructor and implement the relevant plugin interface/base class
...
#### Java
- **Availability**: 3.x only (3.0.0-beta and later)
- **Strongly typed access**: Java plugins operate on typed domain objects such as `TxMetadataLabel` or `AddressUtxo`
- **Use normal Java methods**: Access fields with getters/setters, e.g. `item.getLabel()` or `utxo.getLovelaceAmount()`
- **Use `PluginContext` for utilities**: Access built-ins with `context().namedJdbc()`, `context().http()`, `context().files()`, `context().environment()`, `state()`, and `globalState()`

```java
// Java - Typed domain access
if ("721".equals(item.getLabel())) {
    // process metadata
}
var amount = utxo.getLovelaceAmount();
```
...
- Java: strongly typed objects with normal getters/setters, plus utilities through `PluginContext`
</Callout>
