...
The `PluginFileClient` provides essential file operations designed specifically for plugin developers using MVEL, JavaScript, Python, or Java. It offers simple, secure methods for file I/O, JSON/CSV handling, and directory operations with built-in error handling and sandboxing.
...
// Write processing data
FileOperationResult result = files.write("output.txt", "Item processed: " + itemId);
if (result.isSuccess()) {
    log.info("Data written successfully");
}
...
files.createDir(dateDir);
files.writeJson(dateDir + "/block_" + event.getBlock().getHeader().getHeaderBody().getBlockNumber() + ".json", event.getBlock().getTransactionBodies());
...
```java
// Java - Good: joinPath works on all platforms (files() from the PluginContext)
String logPath = context().files().joinPath("logs", "2024", "01", "app.log");

// Avoid - platform-specific
// String logPath = "logs/2024/01/app.log";   // Unix only
// String logPath = "logs\\2024\\01\\app.log"; // Windows only
```
...
```java
// Java - Choose the appropriate write mode (files() from the PluginContext)
PluginFileClient files = context().files();
...
```java
// Java - Organize data with directories (files() from the PluginContext)
PluginFileClient files = context().files();
...
```java
// Java - Handle missing files gracefully (files() from the PluginContext)
PluginFileClient files = context().files();

// Check existence before reading
if (files.exists("state.json")) {
    FileOperationResult state = files.readJson("state.json");
    // Resume from saved state
} else {
    // Initialize new state
    Map<String, Object> newState = Map.of("startBlock", 0, "processed", 0);
    files.writeJson("state.json", newState);
}
```
...
```java
// Java - All operations return a FileOperationResult (files() from the PluginContext)
FileOperationResult result = context().files().readJson("data.json");

if (result.isSuccess()) {
    Object data = result.getData();     // For JSON/CSV operations
    // or
    String text = result.getAsString(); // For text operations
    // Process data...
} else {
