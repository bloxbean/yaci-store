...
> The Yaci Store plugin system also supports writing plugins in **Python**, **JavaScript**, and **Java**.
However, Python and JavaScript support is currently in **preview**.
For typed, in-app plugins, use **Java** plugins (available in 3.x only) with `lang: java` and `class-name`; see the 
[Java configuration section](./plugin-configuration#java-configuration) and the 
[custom indexer Java example](https://github.com/bloxbean/yaci-store/tree/main/examples/plugins/java/custom-indexer-with-plugins).
...
- [Java Plugin Example App](https://github.com/bloxbean/yaci-store/tree/main/examples/plugins/java/custom-indexer-with-plugins) - Bundled Java plugins in a custom indexer
