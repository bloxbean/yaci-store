...
**Built-in variables and utilities available in plugin scripts and Java plugin contexts**

## What are Context Variables?

Context variables are pre-defined objects that are automatically injected into plugin scripts. Java plugins (3.x only) access the same services through the typed `PluginContext`. They provide instant access to:
- Database operations
...
No imports or initialization needed for script plugins - just use them directly in your MVEL, JavaScript, or Python plugins. In Java plugins, use `context()` / `PluginContext`, for example `context().namedJdbc()`, `context().http()`, `context().files()`, and `state()`.
...
[📖 Full Database Guide →](./variables/database-access)
...
[📖 Full HTTP Client Guide →](./variables/http-client)
...
[📖 Full File Operations Guide →](./variables/file-operations)
...
[📖 Full Configuration Guide →](./variables/configuration)
...
[📖 Full State Management Guide →](./variables/state-management)
...
All context variables work seamlessly across supported scripting languages. Java plugins use equivalent typed accessors on `PluginContext`:
...
<Tabs.Tab>
```java
// Java - Typed PluginContext access
var result = context().namedJdbc().queryForMap(
    "SELECT * FROM block WHERE number = :num",
    java.util.Map.of("num", 100)
);
state().increment("counter");
