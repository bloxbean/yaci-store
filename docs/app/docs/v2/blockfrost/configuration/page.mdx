...
| `blockfrost.apiPrefix` | _(none — required)_ | URL prefix prepended to every yaci-store Blockfrost-compatible controller path. Set by the `blockfrost` profile to `/api/v1/blockfrost`. |
| `blockfrost.hostname` | `""` | Public base URL for this yaci-store instance. Used only by the yaci-store Blockfrost-compatible root endpoint to return an absolute `url` when set. |
...
The profile sets `store.extensions.blockfrost.enabled=true` and overrides `blockfrost.apiPrefix=/api/v1/blockfrost` so the Blockfrost-compatible endpoints don't collide with yaci-store's own `/api/v1/...` controllers.

## API base path

`blockfrost.apiPrefix` is a required property when the extension is enabled — every yaci-store Blockfrost-compatible controller is mapped under `${blockfrost.apiPrefix}`. It has no default; if missing, the application fails to start.
...
The yaci-store Blockfrost-compatible root endpoint (`GET ${blockfrost.apiPrefix}`) returns `blockfrost.hostname + blockfrost.apiPrefix` in its `url` field. Leave `blockfrost.hostname` empty for a relative URL, or set it to this yaci-store instance's public origin when clients expect the root response to contain an absolute URL:

```properties
blockfrost.hostname=https://store.example.com
blockfrost.apiPrefix=/api/v1/blockfrost
```
...
Blockfrost-compatible controllers read from data the underlying yaci-store modules have indexed. The bundled `applications/all` distribution enables every store by default, so no extra wiring is needed.
<Callout type="info">
