...
<Callout type="info">
Analytics Store is available since **2.1.0-pre1** and is part of the **3.x** line (3.0.0-beta and later). It is not included in 2.0.x releases.
</Callout>
...
- **Finality**: a day partition is exported only once every row in it is older than the network's rollback bound — the finality window derived from genesis (`k × slotLength / activeSlotsCoeff`, 12 h on mainnet) plus `finality-margin-hours` (1 h) — and the day is `buffer-days` (1) behind that finalized tip. On mainnet that means each UTC day is exported ~13 h after it ends. See [Configuration](/docs/v2/analytics/configuration).
...
```
analytics-store (write side: exports and catalog ownership)
  admin/           - Admin REST controller and service
  config/          - Spring configuration and properties
  ducklake/        - DuckLake catalog initialization, catalog snapshot reader
...
analytics-query (read side: sandboxed DuckDB engine over the exported data)
  connection/      - Query engine, view registry, unified (live-federated) views
  controller/      - REST API under /api/v1/analytics/query/*
  service/         - Schema discovery, pre-built statistics, SQL validator
  executor/        - Query execution (timeouts, row cap, JSON-safe results)
```

All reads (REST and MCP) go through `analytics-query`; see
[Query API & Unified Views](/docs/v2/analytics/query-api).
