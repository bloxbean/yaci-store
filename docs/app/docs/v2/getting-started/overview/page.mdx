...
However, there are some known mismatches in dRep distribution and **DRep Expiry Epoch**. For the latest mismatch reports, see [the 2.0.0 mismatch summary](../ledger-state-mismatches/2-0-0/overview). We will be working on fixing these in future releases.
