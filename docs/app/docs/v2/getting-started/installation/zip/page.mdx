...
To export blockchain data to Parquet files or DuckLake catalogs for analytical workloads:
...
> **Note:** Analytics Store is available since **2.1.0-pre1**.

Analytics export configuration is in `application-analytics.properties`. Key settings include:
...
- **Export after sync**: `yaci.store.analytics.continuous-sync.export-after-sync` — when `true` (default), exports are deferred until the sync reaches chain tip. **Recommended to keep `true` on mainnet** to optimize resource usage during the sync.
...
1. Edit `application-custom-exporters.yml` to define your custom exporters.
2. Activate with the `custom-exporters` profile:
