...
│   ├── application-analytics.properties   # 3.x only
│   ├── application-custom-exporters.yml   # 3.x only
│   ├── application-blockfrost.properties  # 3.x only
│   ├── application-mcp.yml                # 3.x only
...
*Available in 3.x only (3.0.0-beta and later). Not included in 2.0.x releases.*
...
For storage mode selection, configuration options, custom exporters, and querying exported data, see the [Analytics Store](/docs/v2/analytics/overview) documentation.
