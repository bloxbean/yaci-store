...
- **Storage type**: `yaci.store.analytics.storage.type` — choose `parquet` (default, direct file export) or `ducklake` (managed catalog with ACID transactions and time-travel queries)
