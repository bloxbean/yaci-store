...
│   ├── application-custom-exporters.yml
...
## Enable Analytics Store Profile

To run Yaci Store with the analytics store profile for exporting blockchain data to Parquet/DuckLake:
...
Edit `config/application-analytics.properties` to tune analytics export behavior. Key settings include:

- **Storage type**: `yaci.store.analytics.storage.type` — choose `parquet` (direct file export) or `ducklake` (managed catalog with ACID transactions and time-travel queries). The code default is `parquet`, but the out-of-box `application-analytics.properties` ships with `ducklake`.
- **Export path**: `yaci.store.analytics.export-path` — directory for exported files (default: `./data/analytics`)
- **Admin API**: `yaci.store.analytics.admin.enabled` — enable REST API for manual export control
- **Export after sync**: `yaci.store.analytics.continuous-sync.export-after-sync` — when `true` (default), exports are deferred until the sync reaches chain tip. Set to `false` to export during the sync. **Recommended to keep `true` on mainnet** to optimize resource usage during the sync.

### Custom Exporters

You can define custom SQL-based exporters to export any data from PostgreSQL to Parquet/DuckLake without writing Java code.

1. Edit `config/application-custom-exporters.yml` to define your custom exporters.
2. Add the `custom-exporters` profile to `config/env`:

```env
SPRING_PROFILES_ACTIVE=analytics,custom-exporters
```

Each exporter needs a `name`, a SQL `query` with placeholders (`{source}`, `{start_slot}`, `{end_slot}`, `{epoch}`), and an optional `partition-strategy` (`DAILY` or `EPOCH`).

See the [Analytics Store README](https://github.com/bloxbean/yaci-store/tree/main/aggregates/analytics-store#custom-exporters) for full configuration reference and examples.
