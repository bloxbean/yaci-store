...
*Available in 3.x only.* When the analytics store is enabled, DuckDB memory usage should be explicitly configured to prevent OOM issues in production and container environments. See the [Analytics Store Configuration Reference](/docs/v2/analytics/configuration#duckdb-memory-usage-in-jvm) for details.
...
### Block & Transaction CBOR

#### Enable CBOR bytes in transaction/block payload of Yaci

```properties
store.cardano.return-tx-body-cbor=true
store.cardano.return-block-cbor=true
```

The above properties must be set to true as prerequisites for the following configuration.

The following configs can be used to enable saving of block/transaction CBOR in the database and enable pruning (optional):
