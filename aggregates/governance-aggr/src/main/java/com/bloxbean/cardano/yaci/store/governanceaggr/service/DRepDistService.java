    ...
    public void takeStakeSnapshot(int currentEpoch) {
        ...
        String pv9ClearedAddressesQuery = String.format("""
                ...
                AND EXISTS (
                    ...
                    AND EXISTS (
                        SELECT 1 FROM drep_registration redel_reg
                        ...
                        AND NOT EXISTS (
                            ...
                            AND (   redel_unreg.slot > redel_reg.slot
                                 OR (redel_unreg.slot = redel_reg.slot
                                     AND redel_unreg.tx_index > redel_reg.tx_index)
                                 OR (redel_unreg.slot = redel_reg.slot
                                     AND redel_unreg.tx_index = redel_reg.tx_index
                                     AND redel_unreg.cert_index > redel_reg.cert_index))
                            AND (   redel_unreg.slot < redel.slot
                                 OR (redel_unreg.slot = redel.slot
