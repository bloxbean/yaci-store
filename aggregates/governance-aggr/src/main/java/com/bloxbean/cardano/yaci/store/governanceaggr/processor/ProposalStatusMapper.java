...
public class ProposalStatusMapper {
    ...
    public List<GovActionProposalStatus> mapToProposalStatus(
            GovernanceEvaluationResult result,
            int currentEpoch,
            java.util.Map<GovActionId, ProposalVotingStats> statsMap
    ) {
        return result.getProposalResults().stream()
                .map(proposalResult -> mapSingleProposal(
                        proposalResult,
                        currentEpoch,
                        statsMap == null ? null : statsMap.get(proposalResult.getProposal().getGovActionId())
                ))
                .toList();
    }

    private GovActionProposalStatus mapSingleProposal(
            ProposalEvaluationResult proposalResult,
            int currentEpoch,
            ProposalVotingStats proposalVotingStats
    ) {
        ...
        ProposalVotingStats votingStats = proposalVotingStats == null ? createVotingStats() : proposalVotingStats;
