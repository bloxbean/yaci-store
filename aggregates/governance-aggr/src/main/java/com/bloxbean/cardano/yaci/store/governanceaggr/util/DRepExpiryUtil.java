...
public class DRepExpiryUtil {
     ...
     *
     * <p>{@link #calculateDRepExpiry(DRepRegistrationInfo, DRepInteractionInfo, Set, ProposalSubmissionInfo, int, int)}
     * returns an effective expiry derived from history. {@code cardano-db-sync} stores the raw
     * {@code DRepState.drepExpiry}: proposal transactions flush pending dormant epochs into the stored value,
     * vote/update interactions set it from the current protocol parameters, and later epochs without active
     * proposal state only increase the pending counter. This method mirrors that stateful behavior for
     * {@code active_until} without relying on {@code gov_epoch_activity}.</p>
     */
