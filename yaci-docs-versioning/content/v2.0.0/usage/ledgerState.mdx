...
### ⚙️ Mode 1 - Companion Mode

🔹 **Reads UTxOs from the main database** and **writes account balance** to the **same database**.
...
### ⚙️ Mode 2 - Standalone Mode

🔹 **Writes UTxOs and account balances to a separate database**.

Does not depend on `yaci-store-all` or `utxo-indexer` to run.

➡️ Requires additional `application-aggr.properties` (download from releases).
...
### 🧪 Mode 3 - Standalone with RocksDB (Experimental)

🔹 **Stores UTxOs in embedded RocksDB**, account balances in a **separate database**.

➡️ Still requires `application-aggr.properties`
