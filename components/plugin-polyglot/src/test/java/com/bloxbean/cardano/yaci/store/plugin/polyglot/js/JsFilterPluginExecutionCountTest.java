...
/**
 * Test to verify that filter plugins execute only ONCE per invocation.
 *
 * This test will FAIL before the fix (executionCount = 2)
 * This test will PASS after the fix (executionCount = 1)
 */
class JsFilterPluginExecutionCountTest extends BasePluginTest {

    @Test
    void filterPlugin_shouldExecuteOnlyOnce_notTwice() {
        // Create factory
        JsPolyglotPluginFactory filterFactory = new JsPolyglotPluginFactory(
            null,
            pluginCacheService,
        ...
        filterDef.setName("execution-count-test");
        filterDef.setLang("js");
        filterDef.setInlineScript("""
            // Track how many times this filter executes
            var executionCount = state.get('executionCount') || 0;
            executionCount = executionCount + 1;
            state.put('executionCount', executionCount);

            // Log for debugging
            console.log('Filter executed: ' + executionCount + ' times');
        ...
        var executionCount = pluginCacheService.forPlugin("execution-count-test")
            .get("executionCount");
    ...
    void filterPlugin_multipleInvocations_shouldIncrementCorrectly() {
        ...
        PluginDef filterDef = new PluginDef();
        filterDef.setName("multi-invocation-test");
        filterDef.setLang("js");
        filterDef.setInlineScript("""
        ...
        var count = pluginCacheService.forPlugin("multi-invocation-test")
            .get("count");
